...
class Simulator extends JPanel {	
	...
	protected boolean isGameMode3;
	protected int vertexToColor = 1;
	...
	*/
	protected Simulator (int canvasWidth, int canvasHeight) {
	...
	*/
		...
		public void actionPerformed(ActionEvent e) {
			if (((vertexIndex == 1) && (! vertexColored)) || ((vertexIndex != 1) && (graph.vertices[vertexIndex-1].color != Vertex.DEFAULT_BLANK_COLOR) && !vertexColored)) {
				...
				if (graph.checkIfColorIsLegalForVertex(button.vertex, currentColor)) {
					...
					//Keep track of which vertex has to be drawn in yellow
					vertexToColor ++;
					//Repaint the next VertexButton if there is one
					if (vertexIndex+1 < vertexButtons.length)
						vertexButtons[vertexIndex+1].repaint();
	...
	/** Should use some visual effect to display the next Vertex to be colored in GameMode3
			Used in GameMode3 to show the next Vertex to color and in the Hint function to display the vertex that is hinted at
				Should use variable Positions[][] to get the position of a certain vertex
		
		@param vertexIndex, the index of the vertex that should be marked
	*/
	public void showVertexToColor(int vertexIndex) {
		if (!isGameMode3) {
			//Set showHint to true, and then call repaint on the vertexButton of the Vertex that should be marked
			showHint = true;
			vertexButtons[vertexIndex].repaint();
		}
	}
	...
	*/
	public class VertexButton extends JButton {
		...
		protected void paintComponent(Graphics g) {
			//Graphics2D g2D = (Graphics2D) g;
			g/*2D*/.setColor(getBackground());
			g/*2D*/.fillOval(0, 0, getSize().width - 1, getSize().height - 1);
		...
		protected void paintBorder (Graphics g) {
			...
			//If showHint is true, we draw a yellow border around the button
			if (showHint || (isGameMode3 && vertex.vertexIndex == vertexToColor)) {
